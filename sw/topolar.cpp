...
void	topolar(FILE *fp, FILE *fhp, const char *cmdline, const char *fname, int nstages, int iw, int ow,
...
"\n");
	...
	// Round the results (if necessary)
	// {{{
	...
	if (NULL != fhp) {
		...
		fprintf(fhp, "const double\tGAIN = %.16f;\n",
			cordic_gain(nstages) * sqrt(2.0) / 2.);
